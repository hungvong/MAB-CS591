...
def Mechanism2(epsilon, delta, alpha, gamma):
	...
	while (True):
		...
		if ((Sl >= gamma*(1.+delta)**(-1.)) and (Rl >= R_max)):
			R_max = Rl
			l_max = l
...
#Compute gamma
gamma = np.min((alpha, 1./math.e))

Sl, Rl, pl = Mechanism2(epsilon, delta, alpha, gamma)